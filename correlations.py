...
## data_evals.iloc[:,1:] = data_evals.iloc[:, 1:].apply(stats.zscore)  ### test again with this
...
for row_item in q_items:  # cells containing evaluations
    for col_item in roles:  # cells containing proportions
        ...
        if mdf.pvalues[1] <= 0.05:
            ...
            print("P-value: " + str(mdf.pvalues[1]))                            ### check that correction matches number of comparisons
            print("Bonferroni-corrected P: " + str(mdf.pvalues[1].astype(float) * (len(q_items)*len(roles)))) # 48  is 8 (cols) x 6 (rows predictors)
...
data_ntwrk.iloc[:, 2:].apply(stats.zscore)  # Z-scoring to calculate std. Beta ## not working
...
# Calculate p-values  ## check 1 or 2 colxrow pearson computations to be sure p val is correct
